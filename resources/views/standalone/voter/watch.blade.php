...
<div class="max-w-3xl mx-auto px-4 sm:px-6 py-8">
    ...
    <div class="mb-5 p-5 bg-slate-800/60 border border-slate-700/60 rounded-2xl">
        ...
        <p class="text-slate-400 mt-1.5 text-sm">
            ...
            @if(($campaign->politician->page_published ?? false) && ($campaign->politician->slug ?? false))
                <a href="{{ route('politician.public.show', $campaign->politician->slug) }}"
                   target="_blank" rel="noopener"
                   class="text-emerald-400 font-medium hover:text-emerald-300 transition inline-flex items-center gap-1">
                    {{ $campaign->politician->full_name ?? 'Unknown' }}
