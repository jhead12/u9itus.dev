...
<form method="POST"
    ...
    <div class="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6 space-y-4">
        <h2 class="text-sm font-semibold text-slate-200">Post Content</h2>

        <div>
            <label class="block text-sm font-medium text-slate-300 mb-1.5">Title <span class="text-red-400">*</span></label>
            <input type="text" name="title" value="{{ old('title', $post?->title) }}" required maxlength="120"
    ...
    <div class="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6 space-y-4">
        ...
        <div class="flex flex-wrap gap-2">
            ...
            <label class="inline-flex items-center gap-2 rounded-lg border border-slate-700 bg-slate-900/30 px-3 py-2 text-sm text-slate-300 cursor-pointer hover:border-slate-600">
    ...
    <div class="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6 space-y-4" x-data="{ hasGeo: {{ old('latitude', $post?->latitude) ? 'true' : 'false' }} }">
        ...
        <div x-show="hasGeo" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="sm:col-span-2">
                ...
                <input type="text" name="location_name" value="{{ old('location_name', $post?->location_name) }}"
                    class="w-full bg-slate-900/60 border border-slate-700 rounded-lg px-4 py-2.5 text-white text-sm placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 transition"
            ...
            <div>
                ...
                <input type="number" step="any" name="latitude" value="{{ old('latitude', $post?->latitude) }}"
                    class="w-full bg-slate-900/60 border border-slate-700 rounded-lg px-4 py-2.5 text-white text-sm placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 transition"
                    placeholder="39.7817" />
            </div>
            <div>
                <label class="block text-sm font-medium text-slate-300 mb-1.5">Longitude</label>
                <input type="number" step="any" name="longitude" value="{{ old('longitude', $post?->longitude) }}"
                    class="w-full bg-slate-900/60 border border-slate-700 rounded-lg px-4 py-2.5 text-white text-sm placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 transition"
                    placeholder="-89.6501" />
            </div>
            <div>
            ...
            <div>
                ...
                <input type="text" name="state" value="{{ old('state', $post?->state) }}" maxlength="2"
                    class="w-full bg-slate-900/60 border border-slate-700 rounded-lg px-4 py-2.5 text-white text-sm placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 transition"
                    placeholder="IL" />
            </div>
            <div class="sm:col-span-2">
    ...
    <div class="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6 space-y-4">
        ...
        <div>
            ...
            <input type="url" name="canonical_url" value="{{ old('canonical_url', $post?->canonical_url) }}" maxlength="2048"
                class="w-full bg-slate-900/60 border border-slate-700 rounded-lg px-4 py-2.5 text-white text-sm placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 transition"
    ...
    <div class="flex items-center gap-3">
        <button type="submit"
                class="bg-amber-500 hover:bg-amber-400 text-slate-900 font-semibold rounded-lg px-6 py-2.5 text-sm transition-colors">
