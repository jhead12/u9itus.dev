...
<div class="max-w-6xl mx-auto space-y-6">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
        <p class="text-sm text-slate-400 max-w-xl">
            Arrange the widgets you check most often. Drag a card by its handle to reorder, and
            use the width toggle to give it more room. Your layout is personal — no one else sees it.
        </p>

        @if($availableWidgets->isNotEmpty())
        <form method="POST" action="{{ route('admin.workspace.widgets.store') }}" class="flex items-center gap-2">
            @csrf
            ...
            <button type="submit" class="text-xs font-medium px-3 py-2 rounded-lg bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 hover:bg-emerald-500/20 transition">
                + Add widget
            </button>
        </form>
        @endif
            ...
            <div class="workspace-widget-card {{ $widget->width === 'full' ? 'lg:col-span-2' : '' }} bg-slate-800/50 border border-slate-700/50 rounded-xl overflow-hidden"
                ...
                <div class="px-4 py-3 border-b border-slate-700/50 flex items-center justify-between gap-2 cursor-move widget-drag-handle">
                    ...
                    <div class="flex items-center gap-1 shrink-0">
                        ...
                        <form method="POST" action="{{ route('admin.workspace.widgets.destroy', $widget) }}" onsubmit="return confirm('Remove this widget from your workspace?')">
                            @csrf
                            @method('DELETE')
                ...
                <div class="p-4">
                    @include('standalone.admin.workspace.widgets.'.$widget->widget_key, ['data' => $widget->data])
                </div>
            </div>
        @endforeach
    </div>
...
(function () {
    ...
    function saveLayout() {
        ...
        fetch('{{ route('admin.workspace.widgets.layout') }}', {
            method: 'POST',
            headers: {
