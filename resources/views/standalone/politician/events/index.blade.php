...
<div class="max-w-5xl">
    <div class="flex items-center justify-between mb-6">
        <a href="{{ route('politician.dashboard') }}" class="text-sm text-slate-400 hover:text-white transition">← Back to dashboard</a>
        <a href="{{ route('politician.events.create') }}" class="px-4 py-2 bg-indigo-600 text-white rounded-md text-sm hover:bg-indigo-700">Create Event</a>
    </div>
    ...
    @if($events->isEmpty())
        <div class="bg-slate-800/50 border border-slate-700/50 rounded-xl p-8 text-center">
            ...
            <a href="{{ route('politician.events.create') }}" class="mt-4 inline-block px-5 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">Create your first event</a>
        </div>
    @else
