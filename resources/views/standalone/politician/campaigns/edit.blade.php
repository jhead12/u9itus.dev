...
@endphp
            ...
            <div id="videoFields" class="{{ $editType === 'live_feed' ? 'hidden' : '' }} space-y-4">
                ...
                <div>
                    ...
                    <input type="number" name="media_duration" value="{{ old('media_duration', $campaign->media_duration ?? config('u9itus.min_video_duration', 10)) }}"
                        min="{{ config('u9itus.min_video_duration', 10) }}"
                        max="{{ config('u9itus.max_video_duration', 20) }}"
                        class="w-full bg-slate-900/60 border border-slate-700 rounded-lg px-4 py-2.5 text-white text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 transition" />
                    <p class="text-xs text-slate-500 mt-1">System will auto-detect from video metadata if available ({{ config('u9itus.min_video_duration', 10) }}–{{ config('u9itus.max_video_duration', 20) }}s)</p>
