    ...
    <form method="POST" action="{{ route('politician.campaigns.update', $campaign) }}" enctype="multipart/form-data" class="space-y-6" id="editCampaignForm">
        ...
        <div class="bg-slate-800/50 border border-slate-700/50 rounded-xl p-6 space-y-4">
            ...
            <div>
                ...
                <textarea name="video_blurb" id="videoBlurbInput" class="hidden">{{ old('video_blurb', $campaign->video_blurb) }}</textarea>
            ...
            <div id="videoFields" class="{{ $editType === 'live_feed' ? 'hidden' : '' }} space-y-4">
                ...
                <div>
                    ...
                    <input type="url" name="subtitle_url" id="subtitle_url"
                        ...
                        class="w-full bg-slate-900/60 border border-slate-700 rounded-lg px-4 py-2.5 text-white text-sm placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500 transition" />
                    <p class="text-xs text-slate-500 mt-1">WebVTT (.vtt) file URL. When the platform has subtitles enabled, voters will see captions during playback.</p>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="total_budget" value="{{ old('total_budget', $campaign->total_budget) }}" min="{{ number_format((float) \App\Services\PlatformSettingsService::get('revenue_per_view') * 10, 2, '.', '') }}" step="0.01"
...
<script>
    ...
    const revenuePerViewEdit = {{ (float) ($campaign->revenue_per_view ?? \App\Services\PlatformSettingsService::get('revenue_per_view')) }};
    ...
    function initVideoBlurbEditor() {
        ...
        if (editForm) {
            editForm.addEventListener('submit', syncBlurb);
        }
