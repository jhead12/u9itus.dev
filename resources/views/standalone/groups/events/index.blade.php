...
@section('title', $group->name.' Events')
@section('page-title', $group->name.' Events')
...
<div class="max-w-5xl">
    <div class="flex items-center justify-between mb-6">
        ...
        <a href="{{ route('groups.events.create', $group) }}" class="px-4 py-2 bg-indigo-600 text-white rounded-md text-sm hover:bg-indigo-700">Create Event</a>
    </div>

    @if($events->isEmpty())
        <div class="bg-slate-800/50 border border-slate-700/50 rounded-xl p-8 text-center">
            <p class="text-slate-300">This group hasn't hosted any events yet.</p>
            <a href="{{ route('groups.events.create', $group) }}" class="mt-4 inline-block px-5 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">Create your first event</a>
        </div>
    @else
                    ...
                    @foreach($events as $event)
                        <tr>
                            ...
                            <td class="px-4 py-3 text-right">
                                <a href="{{ route('groups.events.edit', [$group, $event]) }}" class="text-indigo-300 hover:text-indigo-200 text-sm mr-3">Edit</a>
                                @if($event->status->value !== 'cancelled' && $event->status->value !== 'completed')
                                    <form action="{{ route('groups.events.cancel', [$group, $event]) }}" method="POST" class="inline">
                                        @csrf
                                        @method('PATCH')
