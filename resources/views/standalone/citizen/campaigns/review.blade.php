...
@section('title', 'Review: ' . $campaign->title)
@section('page-title', 'Review as Voter')

@section('content')
<div class="max-w-3xl mx-auto px-4 sm:px-6 py-8">

    {{-- Preview mode banner --}}
    <div class="mb-5 rounded-xl border border-blue-500/30 bg-blue-500/10 px-5 py-4 flex items-start gap-3">
        ...
        <div class="flex-1">
            <p class="text-sm font-semibold text-blue-300">Preview Mode</p>
            <p class="text-xs text-slate-400 mt-0.5">
                You are viewing this campaign exactly as a voter will see it. No views or payouts are recorded during preview.
            </p>
        </div>
    </div>
    ...
    <div class="mb-5 flex items-center justify-between">
        <a href="{{ route('citizen.campaigns.show', $campaign) }}" class="inline-flex items-center gap-1.5 text-slate-400 hover:text-white text-sm transition">
    ...
    {{-- Live feed preview notice --}}
    @if($campaign->isLiveFeed())
    <div class="mb-5 rounded-xl border border-amber-500/30 bg-amber-500/10 px-5 py-4">
        <p class="text-sm font-semibold text-amber-300">Live Feed Preview</p>
        <p class="text-xs text-slate-400 mt-1">
            This is a live feed campaign scheduled for {{ $campaign->live_scheduled_at?->format('M j, Y g:i A') ?? 'a future time' }}.
            The stream will only be playable once the campaign is active.
        </p>
    </div>
    @endif

    {{-- Target ZIP / Radius Eligibility --}}
    <div class="mb-5 rounded-xl border {{ $zipMatch ? 'border-emerald-500/30 bg-emerald-500/10' : 'border-amber-500/30 bg-amber-500/10' }} px-5 py-4 flex items-start gap-3">
        <div class="w-9 h-9 rounded-lg {{ $zipMatch ? 'bg-emerald-500/20' : 'bg-amber-500/20' }} flex items-center justify-center shrink-0">
            <svg class="w-5 h-5 {{ $zipMatch ? 'text-emerald-400' : 'text-amber-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/>
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/>
            </svg>
        </div>
        <div class="flex-1">
            <p class="text-sm font-semibold {{ $zipMatch ? 'text-emerald-300' : 'text-amber-300' }}">
                Targeting: {{ $zipMatchLabel }}
            </p>
            <p class="text-xs text-slate-400 mt-0.5">
                {{ $zipMatchReason }}
            </p>
            ...
            <p class="text-xs text-slate-500 mt-1.5">
                Campaign target: <span class="text-slate-300">{{ $campaign->target_zip }}</span>
                @if($campaign->target_zip_radius)
                    &nbsp;&bull;&nbsp; Radius: <span class="text-slate-300">{{ $campaign->target_zip_radius }} miles</span>
                @else
                    &nbsp;&bull;&nbsp; Exact ZIP match
                @endif
            </p>
            @endif
        </div>
    ...
    <div class="relative bg-black rounded-2xl overflow-hidden shadow-2xl ring-1 ring-slate-700/50" id="player-wrapper">
        ...
        <div id="play-overlay" class="absolute inset-0 flex flex-col items-center justify-center bg-black/60 cursor-pointer">
            ...
            <p class="text-white font-semibold">Click to Play Preview</p>
    ...
    @php
        ...
        $canSubmit = $isDraft && ($campaign->media_url || $campaign->live_feed_url);
    ...
    <div class="mt-8 flex flex-col sm:flex-row items-center gap-3">
        <a href="{{ route('citizen.campaigns.show', $campaign) }}"
           class="w-full sm:w-auto px-5 py-2.5 text-sm text-slate-400 hover:text-white border border-slate-700 hover:border-slate-500 rounded-lg transition text-center">
            Back to Campaign
        </a>

        @if($isDraft)
            <a href="{{ route('citizen.campaigns.edit', $campaign) }}"
               class="w-full sm:w-auto px-5 py-2.5 text-sm font-medium text-slate-300 hover:text-white bg-slate-700/50 hover:bg-slate-700 rounded-lg transition text-center">
                Edit Campaign
            </a>
        @endif
        ...
        @if($canSubmit)
            @if($isBallot && empty($campaign->pac_registration_id))
                <span class="w-full sm:w-auto text-center text-xs text-amber-400 bg-amber-500/10 border border-amber-500/20 rounded-lg px-4 py-2.5">
                    Add PAC ID before submitting
                </span>
            @else
                <form method="POST" action="{{ route('citizen.campaigns.submit-review', $campaign) }}" class="w-full sm:w-auto">
                    @csrf
                    <button type="submit"
                        class="w-full px-6 py-2.5 text-sm font-medium text-blue-400 hover:text-blue-300 bg-blue-500/10 hover:bg-blue-500/20 rounded-lg transition">
                        Submit for Review
                    </button>
                </form>
            @endif
...
(function () {
    ...
    function handleCompletion(playbackSeconds) {
        ...
        showStatus('Preview complete. In live mode, a voter who watched this much would qualify for payout.', 'success');
    }
