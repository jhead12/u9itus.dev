...
<div class="space-y-5">
    ...
    <div class="flex items-center gap-3">
        <a href="{{ route('citizen.billing') }}" class="text-sm text-slate-400 hover:text-white transition">← Billing</a>
    </div>
                        ...
                        <tr class="hover:bg-slate-700/10 transition">
                            ...
                            <td class="px-5 py-3 text-right">
                                @if($tx->transaction_type === 'charge' && $tx->status === 'succeeded')
                                    <form method="POST" action="{{ route('citizen.billing.invoices.send-receipt', $tx) }}">
                                        @csrf
                                        <button type="submit"
