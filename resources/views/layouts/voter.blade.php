        ...
        <aside id="voter-sidebar"
            ...
            <nav class="p-3 space-y-0.5 flex-1" aria-label="Voter portal">
                ...
                @php
                    ...
                    $webReporterApproved  = \App\Support\ChatterContributorAccess::allowed(auth()->user());
                    $webReporterRequested = (bool) auth()->user()->chatter_contributor_requested_at;
                    $webReporterRoute     = $webReporterApproved ? 'contributor.chatter.index' : 'voter.onboarding.web-reporter';
                    $webReporterPattern   = $webReporterApproved ? 'contributor.chatter.*' : 'voter.onboarding.web-reporter';
                    $webReporterBadge     = $webReporterApproved ? null : ($webReporterRequested ? 'PENDING' : null);
                    ...
                    $navSections = [
                        ...
                        'Explore' => [
                            ...
                            ['route' => 'voter.favorites.index', 'label' => 'Favorites', 'pattern' => 'voter.favorites.*',
                            ...
                            ['route' => 'voter.causes.index',      'label' => 'Causes',             'pattern' => 'voter.causes.*',
                            ...
                            ['route' => 'voter.ballot-measures.index', 'label' => 'Ballot Measures',  'pattern' => 'voter.ballot-measures.*',
                             'icon' => '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>'],
                            ['route' => 'voter.study-systems', 'label' => 'Study Systems', 'pattern' => 'voter.study-systems',
                        ...
                        'Account' => [
                            ...
                            ['route' => $webReporterRoute, 'label' => 'Web Reporter', 'pattern' => $webReporterPattern, 'badge' => $webReporterBadge,
                            ...
                            ['route' => 'voter.onboarding.welcome', 'label' => 'Replay Onboarding', 'pattern' => 'voter.onboarding.*',
                ...
                @php $isActive = isset($item['route']) && request()->routeIs($item['pattern']); @endphp
                <a href="{{ isset($item['url']) ? $item['url'] : route($item['route']) }}"
                    ...
                    {{ $item['label'] }}
                    @if(! empty($item['badge']))
                    <span class="ml-auto text-[10px] bg-amber-500/20 border border-amber-500/30 text-amber-400 rounded-full px-1.5 py-0.5 leading-none font-semibold">{{ $item['badge'] }}</span>
                    {{-- "New" pill on Running Campaigns when there are available campaigns --}}
                    @elseif(($item['route'] ?? null) === 'voter.ad-room' && ! $isActive)
