...
<title>Campaign Approved — {{ config('app.name', 'U9itus') }}</title>
<style>
...
<div class="wrapper">
  <div class="card">
    ...
    {{-- Body --}}
    <div class="body">
      <div class="status-badge">✅ Campaign Approved</div>

      <h1>Great news, {{ $campaign->politician->user->first_name ?? $campaign->politician->full_name ?? 'Politician' }}!</h1>

      <p>
        Your campaign has been reviewed by our admin team and is now
        <span class="highlight">approved and active</span>. It will begin reaching verified voters
        in your target area immediately.
      </p>
      ...
      <div class="info-box">
        ...
        <div class="info-row">
          ...
          <span class="info-value" style="color:#34d399;">Active</span>
        </div>
        ...
        <div class="info-row">
          <span class="info-label">Target State</span>
          <span class="info-value">{{ strtoupper($campaign->target_state) }}</span>
        </div>
        @endif
        <div class="info-row">
          <span class="info-label">Revenue per View</span>
          <span class="info-value">${{ number_format($revenuePerView ?? \App\Services\PlatformSettingsService::get('revenue_per_view'), 2) }}</span>
        </div>
      </div>

      <p>
        Voters will begin receiving secure notification tokens to watch your video. You'll be
        charged <span class="highlight">${{ number_format($revenuePerView ?? \App\Services\PlatformSettingsService::get('revenue_per_view'), 2) }}</span>
        per verified full view from your credit balance.
      </p>

      <div class="btn-wrap">
        <a href="{{ route('politician.campaigns.show', $campaign->id) }}" class="btn">View Campaign Analytics →</a>
      </div>
      ...
      <p style="font-size:13px">
        ...
        <a href="{{ route('politician.dashboard') }}" style="color:#34d399;text-decoration:none;">politician dashboard</a>
        to manage your campaigns and billing.
      </p>
    </div>
