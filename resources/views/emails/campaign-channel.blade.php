...
<title>{{ $campaign['title'] ?? 'U9itus Campaign' }}</title>
<style>
...
<div class="wrapper">
  <div class="card">
    ...
    <div class="body">
      <h1>{{ $campaign['title'] ?? 'A new campaign for you' }}</h1>
      ...
      <p>{{ $campaign['message_summary'] }}</p>
      @endif

      <p>
        You're receiving this because a campaign on U9itus is reaching voters
        in your area. Watch the full message to <span class="highlight">earn</span>
        and have your voice counted.
      </p>

      <div class="btn-wrap">
        <a href="{{ url('/map') }}" class="btn">View on U9itus →</a>
      </div>
    </div>
