...
<title>You're Off the Waitlist — {{ config('app.name', 'U9itus') }}</title>
<style>
...
<div class="wrapper">
  <div class="card">
    ...
    <div class="body">
      ...
      <p>
        A spot opened up for "<span class="highlight">{{ $event->title }}</span>" and you've been promoted to attending.
      </p>
      ...
      <div class="info-box">
        ...
        <div class="info-row">
          ...
          <span class="info-value">{{ $startsAt->format('l, F j, Y g:i A') }} {{ $event->timezone }}</span>
        </div>
        <div class="info-row">
          <span class="info-label">Location</span>
          <span class="info-value">{{ $event->is_virtual ? 'Virtual' : ($event->venue_name ? $event->venue_name . ', ' . $event->location_name : $event->location_name) }}</span>
        </div>
        <div class="info-row">
          <span class="info-label">Your RSVP</span>
          <span class="info-value">{{ $rsvp->guest_count }} guest{{ $rsvp->guest_count === 1 ? '' : 's' }}</span>
        </div>
        @if($event->is_virtual && $event->virtual_url)
        <div class="info-row">
          <span class="info-label">Virtual URL</span>
          <span class="info-value"><a href="{{ $event->virtual_url }}" style="color:#34d399;">Join here</a></span>
        </div>
        @endif
      </div>

      <div class="btn-wrap">
        <a href="{{ route('events.show', $event->slug) }}" class="btn">View Event Details →</a>
      </div>
