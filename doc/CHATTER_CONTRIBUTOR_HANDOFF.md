...
Implemented in master commit `0bb1b383`. Download: `/downloads/u9itus-source-clipper-0.2.0.zip`. Approved users get installation instructions at `/contribute/chatter/extension`, linked from the contributor form. 0.2.0 adds a `contextMenus`-based "Clip to U9itus" right-click option (page or selection) alongside the existing toolbar-icon flow, both landing on the same review screen in `popup.html`; this needed one small `background.js` service worker (event-driven only — registers the menu item, reacts to its click, nothing else). The extension is an unpacked desktop pilot; no browser-store listing, Firefox/Safari version, or mobile extension has been published. No new database migration is needed.
