...
`GET /api/v1/map/candidate-comparison` accepts optional `context=research`. Default requests retain the existing 90-day election window. Research mode, used by `/compare` and the map's Compare tab, returns current public records any time before the election, allows a null election, and adds structured state/office/district/city fields to `seat`. The comparison assembly is shared through `CandidateComparisonService`. Existing WebMCP contracts are unchanged. Senate and Governor races take the state's election date in years the race calendar (`config/election_races.php`) lists them; a Senate comparison requires that confirmed election and includes only people running in it, since the data has no Senate seat identifiers. Other statewide and local offices require a dated candidate record.
