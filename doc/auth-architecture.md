...
- **Two-factor auth — two independent flows on the same `users` table**:
  ...
  - **Generic SMS recovery (lost authenticator + recovery codes)**: a user with a verified phone (`users.phone_verified_at`) can request a 6-digit SMS code (`App\Services\TwoFactorRecoveryService`, table `two_factor_recovery_sms_codes`, scoped by `user_id` — intentionally separate from the registration-time `phone_verification_codes` table to avoid the two flows clobbering each other's codes) that, once entered, **disables** the account's existing generic 2FA outright rather than passing the challenge — a one-time reset, not an ongoing alternate factor. Routes: `2fa.recovery-sms`, `.send`, `.verify`, `.verify.submit` (`TwoFactorController`), already covered by `EnsureTwoFactorVerified`'s `2fa.*` route-name allowlist. Rate-limited by a dedicated `2fa-recovery-sms` named limiter (3/hour) plus an in-service resend cooldown, both configured under `platform.standalone.auth.two_factor.recovery_sms`. **Admin 2FA has no equivalent** — a locked-out admin still requires the `auth:reset-2fa --admin` artisan command below.
  ...
  - **Manual recovery for stuck accounts (either flow)**: `php artisan auth:reset-2fa {email} [--admin] [--force]` clears the relevant 2FA columns directly — the escape hatch of last resort when SMS recovery isn't available (no verified phone, or an admin account).
