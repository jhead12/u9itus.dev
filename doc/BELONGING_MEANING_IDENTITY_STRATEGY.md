...
| Priority | Item | Notes |
|---|---|---|
| P0 | **Ship Neighborhood Groups (Sprint 8.5 core, not the full Patreon-funding scope)** — group creation, membership, public `/groups/{slug}` page with member count and active causes | Cut scope: ship `neighborhood_groups` + `group_memberships` first; defer `group_contributions`/`group_campaign_budget` (Patreon funding) to a follow-up. Members-only belonging doesn't require money to move first. |
| P0 | **"People near you" signal** — on a favorited Cause or Ballot Measure, show a count (and, opt-in, avatars) of other voters who favorited it in the same district/state | Reuses existing `voter_favorite_causes`/`voter_favorite_ballot_measures` tables — this is a read-query + UI change, no new schema. Highest belonging-value-per-engineering-hour item on this list. |
| P1 | Group badges (`badge_type = group_member`) + group themes, as already spec'd in Creative.md §8.5–8.6 | Build after group membership ships, not before — theming an empty group is pointless. |
| P1 | Lightweight group activity feed — "3 members watched [Politician]'s new campaign this week," "Group hit 50% of its signature-drive goal" | Doesn't require chat/comments (moderation surface); aggregate stats are lower-risk and still create belonging. |
