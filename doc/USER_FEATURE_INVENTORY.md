...
| Follow the Money spotlight | Rotates every half hour between one upcoming ballot measure and the top contested candidates. For a measure (local first, then the visitor's state, then one with committees on both sides; statewide as a fallback): what a YES and a NO vote mean, the verified committees pushing each way, and who funds them. For a candidate (running, with a published profile; money on both sides first, then the visitor's state): the outside groups spending to elect and to defeat them, what they're paying for (from FEC expenditure purposes), and links to each PAC and the candidate's profile. Amounts are secondary detail. Hidden when nothing qualifies. | Homepage, Follow the Money area |
