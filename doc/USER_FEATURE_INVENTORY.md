...
| Ballot-measure funding coverage | Dollar totals are imported nightly for California only (CAL-ACCESS Form 460 summary pages, via `ballot-measures:import-cal-access`); other states show committee links without amounts. Top donors aren't shown, totals are calendar-year only, and late contributions reported after a statement's closing date appear only once the next statement is filed. Most committees don't declare a measure on their filings (70 of 2,624 California Form 460 filers in 2026), so the "Confirmed by filing" check applies to few links. The section appears only after an admin sets the state's finance site or verifies at least one committee. |
