...
| Ballot-measure funding ("Who's Funding This") | See the campaign committees supporting and opposing a measure, each with its state filer ID and a link to the filing, plus a "View official filings" link to the state's campaign finance site. For California, each side shows calendar-year totals (raised, non-cash, spent, cash on hand) from each committee's latest Form 460 plus late contributions reported since, with money passed between committees on the same side counted once, and the side's top 10 donors (with employers as filed). Only admin-verified committee links are shown. | "Who's Funding This" section of `/voter/ballot-measures/{measure}` |
...
| Committee suggestions from filings | For California, see committees that a linked committee funded for the measure (from late contribution reports), with the amount and date, and link one for review or dismiss it. | "Suggested from filings" section of `/admin/ballot-measures/{measure}/committees` |
...
| Ballot-measure funding coverage | Dollar totals, donors and transfers are imported nightly for California only (CAL-ACCESS, via `ballot-measures:import-cal-access`); other states show committee links without amounts. Totals are calendar-year only. Committees filing the short Form 450 show no amounts. Link suggestions come only from late contribution reports (Form 497), not from contributions made earlier in the year (Form 460 Schedule D). Unitemized contributions under $100 are in the totals but not the donor lists. Most committees don't declare a measure on their filings (70 of 2,624 California Form 460 filers in 2026), so the "Confirmed by filing" check applies to few links. The section appears only after an admin sets the state's finance site or verifies at least one committee. |
