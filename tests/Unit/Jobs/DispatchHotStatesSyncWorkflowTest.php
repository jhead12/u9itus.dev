...
test('dispatches workflow_dispatch calls for all four state-scoped workflows', function () {
    config([
    ...
    Http::assertSent(fn ($request) => $request->url() === 'https://api.github.com/repos/HeadEnterprises/u9itus.dev/actions/workflows/refresh-candidate-news.yml/dispatches'
        && $request['inputs']['states'] === 'CA,TX'
    );
});
