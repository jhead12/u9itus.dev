...
test('fix clears a guessed link and its all-null-values summary shape (the production James Gallagher case)', function () {
    $politician = Politician::factory()->create(['full_name' => 'James Gallagher']);

    // normaliseSummary() used to persist this exact all-null shape for a
    // summary table that never parsed — !empty() sees a non-empty array and
    // treats it as "has data" unless hasOpenSecretsSummary() looks inside.
    $snapshot = PoliticianDonorSnapshot::create([
        'politician_id' => $politician->id,
        'opensecrets_source_url' => 'https://www.opensecrets.org/profiles/james-gallagher/us_congress/summary',
    ...
    expect($fresh->opensecrets_source_url)->toBeNull();
    expect($fresh->opensecrets_summary)->toBeNull();
});

test('fix leaves a guessed link untouched when the summary has at least one real value', function () {
    $politician = Politician::factory()->create(['full_name' => 'Jane Doe']);
    ...
    $fresh = $snapshot->fresh();
    expect($fresh->opensecrets_source_url)->not->toBeNull();
    expect($fresh->opensecrets_summary)->not->toBeNull();
});
