...
export const VideoQuestionForm: React.FC<VideoQuestionFormProps> = ({
  ...
  const handleCameraCapture = async () => {
    try {
      const hasPermission = await VideoCaptureService.requestCameraPermission();
      if (!hasPermission) {
      ...
      Alert.alert('Camera', 'Native camera integration coming in next phase');
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const tempPath = await VideoCaptureService.saveTempVideo(selectedVideoPath);
      ...
      if (response.success) {
        ...
        Alert.alert('Success', 'Your video question has been submitted!');
        ...
        Alert.alert('Error', response.message || 'Failed to submit video question');
      }
    } catch (error) {
  ...
  const handleCancel = () => {
    if (selectedVideoPath) {
      Alert.alert(
        'Cancel?',
        'Your video selection will be cleared.',
        [
    ...
    <ScrollView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>
          Send to <Text style={styles.politicianName}>{politicianName}</Text>
        </Text>
      </View>
        ...
        {!selectedVideoPath ? (
          <>
            ...
            >
              ...
              <Text style={styles.videoButtonText}>Record with Camera</Text>
            </TouchableOpacity>
          ...
          <View style={styles.selectedVideo}>
            <Text style={styles.selectedVideoText}>
              ✓ Video selected ({VideoCaptureService.formatFileSize(0)})
            </Text>
            <TouchableOpacity
      ...
      <View style={styles.buttonGroup}>
        ...
        <TouchableOpacity
          style={[
            styles.button,
            styles.submitButton,
            (!selectedVideoPath || isUploading) && styles.disabledButton,
          ]}
      ...
      <Text style={styles.disclaimer}>
        Your question will be emailed to the campaign team and shown in their analytics.
      </Text>
    </ScrollView>
  );
};
