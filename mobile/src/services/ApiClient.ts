...
class ApiClient {
   ...
   */
  async uploadVideoQuestion(token: string, payload: VideoQuestionPayload): Promise<ApiResponse> {
    try {
   ...
   */
  async getPoliticianProfile(campaignId: number): Promise<PoliticianProfile | null> {
    try {
      const response = await this.client.get<ApiResponse<PoliticianProfile>>(
  ...
  async getVideoQuestions(campaignId: number): Promise<VideoQuestion[]> {
    try {
      const response = await this.client.get<ApiResponse<VideoQuestion[]>>(
   ...
   */
  async registerVoter(email: string, password: string, fullName: string): Promise<{ token: string; voter: any } | null> {
    try {
      const response = await this.client.post<ApiResponse>(
        '/voter/register',
   ...
   * Login voter
   */
  async loginVoter(email: string, password: string): Promise<{ token: string; voter: any } | null> {
    try {
      const response = await this.client.post<ApiResponse>(
        '/voter/login',
        { email, password }
